...
namespace WinCopies.Collections.AbstractionInterop.Generic
{
    ...
    {
        public class Queue<TQueue> : IQueue<TDestination> where TQueue : IQueue<TSource>
        {
            ...
            public Queue(in TQueue queue) => InnerQueue = queue;
        ...
        {
            public Queue(in IQueue<TSource> queue) : base(queue)
            {
                // Left empty.
            }
        }
