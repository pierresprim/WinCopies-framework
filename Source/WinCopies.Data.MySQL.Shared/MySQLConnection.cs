...
namespace WinCopies.Data.MySQL
{
    ...
    public class MySQLConnection : SQLConnection<MySqlConnection>, IConnection<MySqlCommand>
    {
        ...
        public MySqlCommand GetCommand(string sql) => new(sql, Connection);
        ...
        public override IInsert GetInsert(string tableName, SQLItemCollection<StringSQLColumn>? columns, SQLItemCollection<SQLItemCollection<IParameter>> values) => new Insert(this, tableName, columns, values);
        ...
        public static ICondition GetCondition<T>(string columnName, SQL.Parameter<T>? parameter, string? tableName = null, string @operator = EQUAL) => new Condition<T>(@operator) { InnerCondition = new ColumnCondition<T>(new KeyValuePair<string?, string>(tableName, columnName), parameter) };

        public override ICondition GetCondition<T>(string columnName, string paramName, T value, string? tableName = null, string @operator = EQUAL) => GetCondition(columnName, new Parameter<T>(paramName, value), tableName, @operator);

        public override ICondition GetNullityCondition(string columnName, string? tableName = null, string @operator = IS) => GetCondition<object?>(columnName, null, tableName, @operator);
        ...
        private static uint? GetResult(int result) => result > -1 ? (uint)result : null;
        ...
        public static void PrepareCommand(MySqlCommand command, IConditionGroup? conditionGroup)
        {
        ...
        public override string? GetOperator(ConditionGroupOperator @operator) => @operator switch
        {
