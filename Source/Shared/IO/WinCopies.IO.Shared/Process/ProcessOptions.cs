...
namespace WinCopies.IO.Process
{
    ...
    public class ProcessOptionsCommon<T> : ProcessOptionsBase<T>
    {
        ...
        public IProcess Process { get => _process; internal set => _process = _process == null ? value ?? throw ThrowHelper.GetArgumentNullException(nameof(value)) : throw new InvalidOperationException("Another process is already registered."); }
        ...
        protected virtual bool UpdateValue<TValue>(ref TValue value, in TValue newValue) => Process?.Status == ProcessStatus.Running
                ...
                : UtilHelpers.UpdateValue(ref value, newValue);

        protected void Dispose() => _process = null;
    }
}
